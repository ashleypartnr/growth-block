...
describe('GreenGrowth Impact Showcase - Interactivity API', () => {
	...
	describe('Filter functionality', () => {
		test('should filter projects by service area', () => {
			// Mock projects data
			const mockProjects = [
				{ id: 1, serviceAreas: ['reforestation'], title: 'Project 1' },
				{ id: 2, serviceAreas: ['carbon-capture'], title: 'Project 2' },
				{ id: 3, serviceAreas: ['reforestation'], title: 'Project 3' },
			];

			// Filter logic that would be in the store
			const filterByArea = (projects, area) => {
				if (area === 'all') return projects;
				return projects.filter(p => p.serviceAreas.includes(area));
			};
	...
	describe('Infinite scroll', () => {
		test('should load more projects when sentinel is reached', () => {
			...
			const loadMore = () => {
				const nextBatch = mockProjects.slice(currentOffset, currentOffset + postsPerPage);
	...
	describe('Card height normalization', () => {
		test('should normalize card heights for alignment', () => {
			...
			const maxTitleHeight = Math.max(...cards.map(c => c.titleHeight));
			const maxExcerptHeight = Math.max(...cards.map(c => c.excerptHeight));
	...
	describe('Accessibility', () => {
		...
		test('active button should have aria-pressed="true"', () => {
			const activeButton = document.querySelector('.gg-filter-button.active');
